...
if ($_POST) {
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'guardar_vertimiento') {
            try {
                ...
                $actividad_meta = [
                    'No Retronable (NR)' => 'Vertimiento NR',
                    'Retronable (R)' => 'Vertimiento R',
                    'PET-TW' => 'Vertimiento PET'
                ];
                
                $meta_buscar = $actividad_meta[$_POST['actividad']];
                
                $stmt = $pdo->prepare("SELECT meta_minima, disparador FROM metas WHERE actividad = ?");
                $stmt->execute([$meta_buscar]);
                $meta = $stmt->fetch();
                
                $cumple_meta = $cumplimiento_general >= $meta['meta_minima'] ? 1 : 0;
                $estado_ciclo = $cumple_meta ? 'completo' : 'pendiente';
                
                $grupo_registro = uniqid('vert_');
                ...
                foreach ($_POST['productos'] as $producto) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO vertimiento (fecha, auxiliar_id, nombre, actividad, turno, producto_id, sku, producto_nombre, unidades, hora_inicio, hora_fin, horas_trabajadas, cumplimiento_individual, cumplimiento_general, cumple_meta, estado_ciclo, observaciones, grupo_registro, operacion_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([
                        $_POST['fecha'],
                        $_SESSION['usuario_id'],
                        $_SESSION['nombre'],
            ...
            try {
                if (isset($_FILES['evidencia']) && $_FILES['evidencia']['error'] == 0) {
                    ...
                    if (move_uploaded_file($_FILES['evidencia']['tmp_name'], $file_path)) {
                        $stmt = $pdo->prepare("UPDATE vertimiento SET evidencia_5_porque = ?, estado_ciclo = 'completo' WHERE grupo_registro = ? AND operacion_id = ?");
...
$stmt = $pdo->query("SELECT actividad, meta_minima, disparador FROM metas WHERE actividad IN ('Vertimiento NR', 'Vertimiento R', 'Vertimiento PET')");
$metas = [];
    ...
    <div class="container">
        <div class="page-header">
            <h1>
                ...
                Gestión de Productividad - Vertimiento
            </h1>
            <p>Registra y gestiona la productividad de las actividades de vertimiento</p>
        </div>
        ...
        <div class="records-table">
            <table id="tablaVertimiento" class="display responsive nowrap" style="width:100%">
                <thead>
                    <tr>
                        <th>Fecha</th>
                        <th>Actividad</th>
                    ...
                    <?php foreach ($registros as $registro): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                $actividad_meta_display = [
                                    'No Retronable (NR)' => 'Vertimiento NR',
                                    'Retronable (R)' => 'Vertimiento R',
                                    'PET-TW' => 'Vertimiento PET'
                                ];
                                
                                $meta_key = $actividad_meta_display[$registro['actividad']];
                                $meta = $metas[$meta_key]['meta'];
                                $disparador = $metas[$meta_key]['disparador'];
    ...
    <div id="modalNuevo" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2><i class="fas fa-plus-circle"></i> Nueva Productividad Vertimiento</h2>
                <span class="close" onclick="closeModal()">&times;</span>
            </div>
            <form id="formProductividad" method="POST">
                <input type="hidden" name="action" value="guardar_vertimiento">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select name="actividad" class="form-select" required onchange="updateMeta()">
                                    ...
                                    <option value="No Retronable (NR)">No Retronable (NR)</option>
                                    <option value="Retronable (R)">Retronable (R)</option>
                                    <option value="PET-TW">PET-TW</option>
                                </select>
                            </div>
                            <div class="form-group">
    ...
    <script>
        ...
        function calculateCumplimiento() {
            ...
            productos.forEach(producto => {
                ...
                if (horaInicio && horaFin && unidades > 0 && fechaBase) {
                    totalUnidades += unidades;
                    
                    const inicio = new Date(fechaBase + 'T' + horaInicio);
                    let fin = new Date(fechaBase + 'T' + horaFin);
        ...
        function updateMeta() {
            ...
            if (actividad && actividadToMeta[actividad] && metas[actividadToMeta[actividad]]) {
                const meta = metas[actividadToMeta[actividad]];
        ...
        function verDetalle(grupoRegistro) {
            fetch('../api/vertimiento/get_detalle_vertimiento.php?grupo=' + grupoRegistro)
