...
if ($_POST) {
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'guardar_revision') {
            try {
                ...
                $actividad_meta = [
                    'No Retornable (NR)' => 'Revisión NR',
                    'Retornable' => 'Revisión',
                    'PET-TW' => 'Revisión',
                    'Envase' => 'Revisión de envase'
                ];
                ...
                foreach ($_POST['productos'] as $producto) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO revision (fecha, auxiliar_id, nombre, actividad, turno, producto_id, sku, producto_nombre, unidades, hora_inicio, hora_fin, horas_trabajadas, cumplimiento_individual, cumplimiento_general, cumple_meta, estado_ciclo, observaciones, grupo_registro, operacion_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([
                        $_POST['fecha'],
                        $_SESSION['usuario_id'],
                        $_SESSION['nombre'],
            ...
            try {
                if (isset($_FILES['evidencia']) && $_FILES['evidencia']['error'] == 0) {
                    ...
                    if (move_uploaded_file($_FILES['evidencia']['tmp_name'], $file_path)) {
                        $stmt = $pdo->prepare("UPDATE revision SET evidencia_5_porque = ?, estado_ciclo = 'completo' WHERE grupo_registro = ? AND operacion_id = ?");
...
$stmt = $pdo->query("SELECT actividad, meta_minima, disparador FROM metas WHERE actividad IN ('Revisión NR', 'Revisión', 'Revisión de envase')");
$metas = [];
...
<head>
    ...
    <title>Productividad Revisión - Ware-Pro</title>
...
<body>
    <div class="container">
        <div class="page-header">
            <h1>
                ...
                Gestión de Productividad - Revisión
            </h1>
            <p>Registra y gestiona la productividad de las actividades de revisión</p>
        </div>
        ...
        <div class="records-table">
            <table id="tablaRevision" class="display responsive nowrap" style="width:100%">
                <thead>
                    <tr>
                        <th>Fecha</th>
                        <th>Actividad</th>
    ...
    <div id="modalNuevo" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2><i class="fas fa-plus-circle"></i> Nueva Productividad Revisión</h2>
                <span class="close" onclick="closeModal()">&times;</span>
            </div>
            <form id="formProductividad" method="POST">
                <input type="hidden" name="action" value="guardar_revision">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select name="actividad" class="form-select" required onchange="updateMeta()">
                                    ...
                                    <option value="No Retornable (NR)">No Retornable (NR)</option>
    ...
    <script>
        ...
        function verDetalle(grupoRegistro) {
            fetch('../../api/revision/get_detalle_revision.php?grupo=' + grupoRegistro)
