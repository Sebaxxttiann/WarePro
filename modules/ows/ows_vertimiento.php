    ...
    try {
        switch ($action) {
            ...
            case 'editar':
                ...
                $stmt = $pdo->prepare("UPDATE ows_vertimiento SET fecha = ?, hora = ?, actividad = ?, unidades = ?, updated_at = NOW() WHERE id = ? AND operacion_id = ?");
            ...
            case 'eliminar':
                ...
                $stmt = $pdo->prepare("DELETE FROM ows_vertimiento WHERE id = ? AND operacion_id = ?");
            ...
            case 'obtener':
                ...
                $stmt = $pdo->prepare("SELECT * FROM ows_vertimiento WHERE id = ? AND operacion_id = ?");
                $stmt->execute([$id, getOperacionActiva()]);
                $registro = $stmt->fetch();
...
<head>
    ...
    <style>
        * {
        ...
        #vertimientoTable thead th i {
            margin-right: 8px;
            color: var(--primary-color);
        }

        #vertimientoTable tbody td {
            padding: 1rem !important;
        ...
        #vertimientoTable tbody tr:hover td {
...
<div class="main-container">
    ...
    <div class="content-container">
        <div class="table-container">
            ...
            <table id="vertimientoTable" class="display">
                <thead>
                    <tr>
...
<script>
    ...
    function makeRequest(action, data = {}) {
        ...
        return fetch('ows_vertimiento.php', {
            method: 'POST',
            body: formData
    ...
    function initDataTable() {
        dataTable = $('#vertimientoTable').DataTable({
            language: {
            ...
            columns: [
                ...
                { 
                    data: 'actividad',
                    render: function(data) {
                        return `<span style="background: #17a2b8; color: white; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 600;">
                                    ${data}
                                </span>`;
                    }
                },
